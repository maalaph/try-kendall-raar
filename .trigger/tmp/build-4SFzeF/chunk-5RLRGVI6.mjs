...
async function searchSimilarEmbeddings(recordId, queryEmbedding, options = {}) {
  try {
    ...
    if (error) {
      ...
      let query = supabase.from("embeddings").select("*").eq("record_id", recordId).not("embedding", "is", null).order("embedding", { ascending: true, foreignTable: "embeddings" }).limit(limit);
      if (options.threadId) {
...
//# sourceMappingURL=chunk-5RLRGVI6.mjs.map
