...
async function createChatMessage(data) {
  try {
    const { data: threadExists, error: threadCheckError } = await supabase.from("threads").select("thread_id").eq("thread_id", data.threadId).single();
...
async function getUserMemories(recordId, memoryType, importance) {
  try {
    let query = supabase.from("user_memories").select("*").eq("record_id", recordId).order("importance", { ascending: false }).order("created_at", { ascending: false });
    if (memoryType) {
...
//# sourceMappingURL=chunk-Y6ECJARP.mjs.map
