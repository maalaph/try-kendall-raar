...
// trigger/embedding-tasks.ts
init_esm();

// lib/semanticMemory.ts
init_esm();
...
async function generateBatchEmbeddings(contents) {
...
__name(generateBatchEmbeddings, "generateBatchEmbeddings");

// lib/semanticMemory.ts
init_database();
init_database();
async function indexMessage(recordId, threadId, messageId, content, metadata) {
  try {
    if (!content || content.trim().length === 0) {
      return null;
    }
...
async function indexMemory(recordId, memory) {
  try {
...
__name(indexMemory, "indexMemory");
...
__name(batchIndexMessages, "batchIndexMessages");

// trigger/embedding-tasks.ts
init_database();
init_database();
var indexMessageTask = task({
  id: "index-message",
...
var batchIndexMessagesTask = task({
...
var backfillUserEmbeddingsTask = task({
...
export {
  indexMessageTask,
  batchIndexMessagesTask,
  backfillUserEmbeddingsTask,
  indexMemoryTask,
  indexAllMemoriesTask
};
//# sourceMappingURL=chunk-W35DMXCS.mjs.map
