...
async function getUserMemories(recordId, memoryType, importance) {
  try {
    let query = supabase.from("user_memories").select("*").eq("record_id", recordId).order("created_at", { ascending: false });
    let data, error;
    try {
    ...
    if (error) {
      if (error.message && error.message.includes("importance")) {
        console.warn('[DATABASE] user_memories table missing "importance" column. Run schema migration.');
        query = supabase.from("user_memories").select("*").eq("record_id", recordId).order("created_at", { ascending: false });
        if (memoryType) {
    ...
    if (data && data.length > 0 && "importance" in data[0]) {
      if (memoryType) {
        data = data.filter((m) => m.memory_type === memoryType);
      }
      if (importance) {
...
//# sourceMappingURL=chunk-5YXU2WW5.mjs.map
